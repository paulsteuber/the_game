    ...
    <div className="card-wrapper m-2 p-2 d-flex align-items-center flex-column">
      <div className="card-header shadow">
        <div className="btn-group" role="group" aria-label="Basic example">
          <button type="button" className={buttonStatus[0].className+" btn btn-sm"} onClick={() =>{addCardToStack(0)}}>A</button>
          <button type="button" className={buttonStatus[1].className+" btn btn-sm"} onClick={() =>{addCardToStack(1)}}>B</button>
          <button type="button" className={buttonStatus[2].className+" btn btn-sm"} onClick={() =>{addCardToStack(2)}}>C</button>
          <button type="button" className={buttonStatus[3].className+" btn btn-sm"} onClick={() =>{addCardToStack(3)}}>D</button>
        </div>
      </div>
      <div className="user-card rounded-2 shadow py-4 m-2 d-flex justify-content-center align-items-center">
        <span className="h1 fw-bolder m-0 p-4">{cardNumber.value}</span></div>
    </div>
    );
}